...
const SessionInscriptions = () => {
  ...
  useEffect(() => {
    const charger = async () => {
      if (!session) {
        setAlerte({ type: 'danger', message: 'Données de session manquantes.' });
        setLoading(false);
        return;
      }
  ...
  const handleDelete = async (id, matricule) => {
    if (!window.confirm(`Supprimer l'inscription de ${matricule || 'cet étudiant'} ?`)) return;
    try {
  ...
  return (
    <div className="container-fluid p-0">
      ...
      <main className="container mt-4" style={{ maxWidth: '80%' }}>
        <h1 className="text-center mb-3" style={{ fontSize: '24px', fontWeight: 'bold', color: '#343a40' }}>
          • {session?.intituleCours} • {session?.anneeAcademique} • Semestre {session?.semestre} • {session?.codeProgramme}
        </h1>
              ...
              <tbody>
                {inscriptions.map((insc) => (
                  <tr key={insc.id} style={{ border: '1px solid black' }}>
                    ...
                    <td>{insc.nom || '—'}</td>
                    <td>{insc.prenoms || '—'}</td>
                    <td>{insc.moyenneFinale != null ? insc.moyenneFinale : '—'}</td>
                    <td>{insc.sanction || '—'}</td>
                    <td>
                      <button className="btn btn-primary btn-sm me-2" onClick={() => navigate(`/edit-inscription?id=${insc.id}`)}>
                        Choisir
                      </button>
                      <button className="btn btn-danger btn-sm" onClick={() => handleDelete(insc.id, insc.matricule)}>
                        🗑️
                      </button>
                    </td>
                  </tr>
